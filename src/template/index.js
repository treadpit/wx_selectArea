  ...
  bindChange: function (e) {
    ...
    if (current_value.length > 2) {
      ...
      if (value[0] !== current_value[0] && value[1] === current_value[1] && value[2] === current_value[2]) {
        // 滑动省份
        fetch(wx.request, {
          url: API + proviceData[current_value[0]].code,
          method: 'GET'
        }).then((city) => {
					...
					if (cityData && cityData.length) {
						...
						this.setData({
							...
							'areaPicker.address': proviceData[current_value[0]].fullName,
							'areaPicker.selected': [proviceData[current_value[0]]],
						});
					}
          ...
          if (districtData && districtData.length > 0) {
            ...
            this.setData({
              ...
              'areaPicker.value[0]': current_value[0],
              'areaPicker.value[1]': 0,
							'areaPicker.value[2]': 0,
              'areaPicker.address': proviceData[current_value[0]].fullName + ' - ' + cityData[0].fullName + ' - ' + districtData[0].fullName,
							'areaPicker.selected': [proviceData[current_value[0]], cityData[0], districtData[0]]
            });
          } else {
						this.setData({
              'areaPicker.districtData': [],
              'areaPicker.value[0]': current_value[0],
              'areaPicker.value[1]': current_value[1],
              'areaPicker.value[2]': 0,
							'areaPicker.address': proviceData[current_value[0]].fullName + ' - ' + cityData[0].fullName,
							'areaPicker.selected': [proviceData[current_value[0]], cityData[0]]
            });
					}
        }).catch((e) => {
          console. error(e);
        });
      } else if (value[0] === current_value[0] && value[1] !== current_value[1] && value[2] === current_value[2]) {
				const { proviceData, cityData } = this.data.areaPicker;
        // 滑动城市
        fetch(wx.request, {
          url: API + cityData[current_value[1]].code,
          method: 'GET'
        }).then((district) => {
					if (!district) return;
					const districtData = district.data.result;
          if (districtData && districtData.length > 0) {
            ...
            this.setData({
              ...
              'areaPicker.value[1]': current_value[1],
              'areaPicker.value[2]': 0,
							'areaPicker.address': proviceData[current_value[0]].fullName + ' - ' + cityData[current_value[1]].fullName + ' - ' + districtData[0].fullName,
							'areaPicker.selected': [proviceData[current_value[0]], cityData[current_value[1]], districtData[0]]
            });
          } else {
						this.setData({
              'areaPicker.districtData': [],
              ...
              'areaPicker.value[1]': current_value[1],
              'areaPicker.value[2]': 0,
							'areaPicker.address': proviceData[current_value[0]].fullName + ' - ' + cityData[current_value[1]].fullName,
							'areaPicker.selected': [proviceData[current_value[0]], cityData[current_value[1]]]
            });
					}
        }).catch((e) => {
          console.error(e);
        ...
        // 滑动地区
        this.setData({
          'areaPicker.value': current_value,
					'areaPicker.address': proviceData[current_value[0]].fullName + ' - ' + cityData[current_value[1]].fullName + ' - ' + districtData[current_value[2]].fullName,
					'areaPicker.selected': [proviceData[current_value[0]], cityData[current_value[1]], districtData[current_value[2]]]
        });
      }
    }
  }
